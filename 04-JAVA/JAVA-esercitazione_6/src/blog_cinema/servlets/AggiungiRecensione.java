		...
		if(utente.getTipo() != TipoUtente.ADMIN) {
			...
			if (request.getParameter("Salva") != null) {
				...
				if (request.getParameter("film") != null) {
					film = FilmFactory.get_instance().get_film(Integer.parseInt(request.getParameter("film")));
				}
				...
				if(film != null && contenuto_recensione != null && voto != null) {
					if(request.getParameter("id")==null) {
						try {
							...
							recensione.setId(RecensioneFactory.get_instance().getRecensioni().size());
							recensione.setFilm(film);
							recensione.setContenuto_recensione(contenuto_recensione);
							recensione.setVoto(voto);
							recensione.setAutore(utente);
							
							RecensioneFactory.get_instance().getRecensioni().add(recensione);
						...
						try {
							recensione = RecensioneFactory.get_instance().get_recensione(id);
							recensione.setFilm(film);
							recensione.setContenuto_recensione(contenuto_recensione);
							recensione.setVoto(voto);
						} catch (Exception e) {
				...
				if (temp_id!=null) {
					...
					try {
						...
						if (recensione != null && recensione.getAutore().equals(session.getAttribute("utente"))) {
							RecensioneFactory.get_instance().getRecensioni().remove(recensione);
